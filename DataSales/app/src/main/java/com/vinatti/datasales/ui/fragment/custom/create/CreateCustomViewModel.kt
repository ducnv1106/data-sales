...
import com.vinatti.datasales.data.api_entities.response.ResponseAddCustom
...
@HiltViewModel
class  CreateCustomViewModel @Inject constructor(private val gson: Gson, private val apiController: ApiController) : ViewModel() {
    ...
    // request add custom
    private val _addCustom : MutableLiveData<RequestObject> = MutableLiveData()
    fun requestAddCustom(request : RequestObject) {
        _addCustom.value = request
    }
    private val _requestAddCustomResult : LiveData<ApiResult<ResponseAddCustom.AddCustomResult>> = Transformations.switchMap(_addCustom) {
        performGetOperation(gson){
            apiController.addCustom(it)
        }
    }
    val requestAddCustomResult : LiveData<ApiResult<ResponseAddCustom.AddCustomResult>> = _requestAddCustomResult
